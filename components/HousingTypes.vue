<template>
  <div class="transition-all"
    :class="housing_list_expanded ? 'w-[calc(100%_+_2rem)] -ml-4 bg-gray-100 rounded-xl overflow-hidden shadow-xl' : ''">
      ...
      <div class="transition-all"
        ...
        <div v-for="(h, i) in housing_types" :key="i" :id="`h-${i}`"
          ...
          :class="housing_list_expanded ?
            ('rounded-md hover:shadow hover:shadow-gray-400 hover:dark:shadow-black px-4 py-3 gap-1 grow ' + (i == selected_id ? 'bg-gray-100' : 'bg-gray-200')) : 'rounded-xl hover:shadow-xl hover:-translate-y-1 p-4 sm:p-8 gap-4 bg-gray-100'">
    ...
    <div class="mt-4 space-y-4 px-4 pb-6" v-if="housing_list_expanded">
      ...
      <p class="text-gray-500">
        <text v-for=" n in housing_types[selected_id].notices">{{ n.trim().replace(/\.$/, "") + ". " }}</text>
      </p>
      <div class="grid grid-cols-2 sm:grid-cols-4 gap-2" style="
            grid-template-areas: 
                'room_count room_size room_size furnished'
                'rent rent waiting_period waiting_period';
            ">
        <div class="bg-gray-200 p-4 rounded-xl flex flex-col justify-center gap-1" :style="{ 'grid-area': 'room_count' }">
          <div class="font-bold text-xl leading-none">{{ housing_types[selected_id].room_count }}</div>
          <div class="text-gray-500">Zimmer</div>
        </div>
        <div class="bg-gray-200 p-4 rounded-xl flex flex-col justify-center gap-1" :style="{ 'grid-area': 'room_size' }">
          <div class="font-bold text-xl leading-none">{{ housing_types[selected_id].room_size }}</div>
          <div class="text-gray-500">Zimmergröße</div>
        </div>
        <div class="bg-gray-200 p-4 rounded-xl flex flex-col justify-center gap-1" :style="{ 'grid-area': 'furnished' }">
          <div class="font-bold text-xl leading-none capitalize">{{ housing_types[selected_id].furnished }}
          </div>
        </div>
        <div class="bg-gray-200 p-4 rounded-xl flex flex-col justify-center gap-1" :style="{ 'grid-area': 'rent' }">
          <div class="font-bold text-xl leading-none">{{ housing_types[selected_id].rent }}</div>
          <div class="text-gray-500">Miete</div>
        </div>
        <div class="bg-gray-200 p-4 rounded-xl flex flex-col justify-center gap-1"
          :style="{ 'grid-area': 'waiting_period' }">
          <div class="font-bold text-xl leading-none">{{ housing_types[selected_id].waiting_period }}</div>
          <div class="text-gray-500">Wartezeit</div>
        </div>
      </div>
      <ul class="text-gray-500 list-disc pl-4 space-y-0">
